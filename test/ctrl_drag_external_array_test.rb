require 'minitest/autorun'
require 'stringio'
require_relative 'support/fixtures'

# Coverage for the Ctrl-drag EXTERNAL copy array: Ctrl-drag a Gizmo move
# arrow to create the ordinary single endpoint copy (pre-existing
# behavior, untouched by this feature), then immediately type "x3" (or
# "3x"/"X3"/"3X") in Measurements to repeat the complete dragged distance
# that many times, creating that many total copies (the endpoint counts
# as the first of them) spaced at the original drag distance.
#
# This shares the exact @ctrl_array_session architecture, transparent-undo
# strategy, and duplicate_supported_entity/copy_array_selection_issue
# helpers as the internal "/N" array (test/ctrl_drag_internal_array_test.rb)
# -- see GizmoOverlay#apply_ctrl_drag_array, which both syntaxes now share,
# distinguished only by a `mode` argument (:internal vs :external) that
# picks the position formula. Tests drive the real callbacks the same way
# the internal-array tests do: TestHarness.drive_move_drag simulates the
# gizmo's own on_transform_start/on_transform/on_transform_end sequence
# for a Ctrl-held move, then GizmoOverlay#onUserText is called directly
# with the typed Measurements text.
class CtrlDragExternalArrayTest < Minitest::Test
  def setup
    @model = Sketchup::Model.new
    Zbellbound::SmartGizmoPro::PLUGIN.test_smart_scale_enabled = false
    Zbellbound::SmartGizmoPro::PLUGIN.test_gizmo_orientation = Zbellbound::SmartGizmoPro::GLOBAL_ORIENTATION
    UI.last_messagebox_text = nil
    UI.last_inputbox_args = nil
  end

  def select(*entities)
    @model.selection.instance_variable_set(:@list, entities)
  end

  def build_overlay
    TestHarness.build_overlay(model: @model, selection: @model.selection)
  end

  def assert_transformation_equal(expected, actual, msg = nil)
    expected.to_a.each_with_index do |value, i|
      assert_in_delta value, actual.to_a[i], 1e-6, msg || "transformation component #{i} mismatch"
    end
  end

  def operation_counts
    starts = @model.operation_log.count { |entry| entry[0] == :start }
    commits = @model.operation_log.count { |entry| entry[0] == :commit }
    aborts = @model.operation_log.count { |entry| entry[0] == :abort }
    [starts, commits, aborts]
  end

  def ctrl_drag_copy(entities, vector)
    select(*entities)
    overlay = build_overlay
    TestHarness.drive_move_drag(overlay, vector: vector, copy: true)
    overlay
  end

  def session_of(overlay)
    overlay.instance_variable_get(:@ctrl_array_session)
  end

  # -- Parsing: xN, Nx, case-insensitive, whitespace ------------------------

  def test_parse_ctrl_drag_external_array_count_accepts_all_syntax_variants
    overlay = build_overlay

    assert_equal 3, overlay.parse_ctrl_drag_external_array_count('x3')
    assert_equal 3, overlay.parse_ctrl_drag_external_array_count('3x')
    assert_equal 3, overlay.parse_ctrl_drag_external_array_count('X3')
    assert_equal 3, overlay.parse_ctrl_drag_external_array_count('3X')
    assert_equal 3, overlay.parse_ctrl_drag_external_array_count(' x3 ')
    assert_equal 3, overlay.parse_ctrl_drag_external_array_count(' 3x ')
    assert_equal 3, overlay.parse_ctrl_drag_external_array_count('x 3')
    assert_equal 3, overlay.parse_ctrl_drag_external_array_count('3 x')
    assert_equal 1, overlay.parse_ctrl_drag_external_array_count('x1')
    assert_equal 1000, overlay.parse_ctrl_drag_external_array_count('x1000')
  end

  def test_parse_ctrl_drag_external_array_count_returns_nil_for_non_x_text
    overlay = build_overlay

    assert_nil overlay.parse_ctrl_drag_external_array_count('300mm')
    assert_nil overlay.parse_ctrl_drag_external_array_count('3')
    assert_nil overlay.parse_ctrl_drag_external_array_count('/3'), 'internal-array syntax must not match here'
    assert_nil overlay.parse_ctrl_drag_external_array_count('')
    assert_nil overlay.parse_ctrl_drag_external_array_count(nil)
  end

  def test_parse_ctrl_drag_external_array_count_rejects_malformed_input
    overlay = build_overlay

    assert_equal :invalid, overlay.parse_ctrl_drag_external_array_count('x0')
    assert_equal :invalid, overlay.parse_ctrl_drag_external_array_count('x-2')
    assert_equal :invalid, overlay.parse_ctrl_drag_external_array_count('x2.5')
    assert_equal :invalid, overlay.parse_ctrl_drag_external_array_count('x')
    assert_equal :too_many, overlay.parse_ctrl_drag_external_array_count('x1001')
  end

  # -- xN creates the correct spacing, reusing the existing endpoint ------

  def test_x3_creates_two_additional_copies_beyond_the_existing_endpoint
    group = TestFixtures.group_box
    overlay = ctrl_drag_copy([group], Geom::Vector3d.new(1000.mm, 0, 0))
    endpoint = @model.selection.to_a.first

    overlay.onUserText('x3', @model.active_view)

    selected = @model.selection.to_a
    assert_equal 4, selected.length, 'original + endpoint (copy 1) + 2 more copies'
    assert_includes selected, group
    assert_includes selected, endpoint, 'the endpoint copy must be reused, never recreated'

    xs = selected.map { |e| e.transformation.origin.x }.sort
    assert_in_delta 0, xs[0], 1e-6
    assert_in_delta 1000.mm, xs[1], 1e-6, 'endpoint stays at the original dragged distance (copy 1)'
    assert_in_delta 2000.mm, xs[2], 1e-6
    assert_in_delta 3000.mm, xs[3], 1e-6
  end

  def test_suffix_and_uppercase_x_syntax_produce_the_same_array
    [
      '3x',
      'X3',
      '3X'
    ].each do |text|
      group = TestFixtures.group_box
      overlay = ctrl_drag_copy([group], Geom::Vector3d.new(1000.mm, 0, 0))

      overlay.onUserText(text, @model.active_view)

      xs = @model.selection.to_a.map { |e| e.transformation.origin.x }.sort
      assert_equal 4, xs.length, "for #{text.inspect}"
      assert_in_delta 0, xs[0], 1e-6, "for #{text.inspect}"
      assert_in_delta 1000.mm, xs[1], 1e-6, "for #{text.inspect}"
      assert_in_delta 2000.mm, xs[2], 1e-6, "for #{text.inspect}"
      assert_in_delta 3000.mm, xs[3], 1e-6, "for #{text.inspect}"
    end
  end

  def test_x1_on_a_fresh_copy_is_a_no_op
    group = TestFixtures.group_box
    overlay = ctrl_drag_copy([group], Geom::Vector3d.new(1000.mm, 0, 0))
    starts_before, commits_before, = operation_counts

    overlay.onUserText('x1', @model.active_view)

    starts_after, commits_after, aborts_after = operation_counts
    assert_equal starts_before, starts_after, 'x1 when already at count 1 has nothing to do'
    assert_equal commits_before, commits_after
    assert_equal 0, aborts_after
    assert_equal 1, @model.selection.to_a.length
  end

  def test_x1_after_x3_removes_the_extra_copies_leaving_only_the_endpoint
    group = TestFixtures.group_box
    overlay = ctrl_drag_copy([group], Geom::Vector3d.new(1000.mm, 0, 0))
    endpoint = @model.selection.to_a.first
    overlay.onUserText('x3', @model.active_view)
    extras = session_of(overlay)[:intermediates].dup
    assert_equal 2, extras.length

    overlay.onUserText('x1', @model.active_view)

    assert extras.none?(&:valid?), 'the x3 extra copies must be erased'
    selected = @model.selection.to_a
    assert_equal 2, selected.length, 'original + endpoint only'
    assert_includes selected, endpoint
    assert_equal 0, session_of(overlay)[:intermediates].length
  end

  # -- Directions -----------------------------------------------------------

  def test_x3_handles_positive_and_negative_directions_on_every_axis
    [
      Geom::Vector3d.new(600.mm, 0, 0),
      Geom::Vector3d.new(-600.mm, 0, 0),
      Geom::Vector3d.new(0, 450.mm, 0),
      Geom::Vector3d.new(0, -450.mm, 0),
      Geom::Vector3d.new(0, 0, 900.mm),
      Geom::Vector3d.new(0, 0, -900.mm)
    ].each do |vector|
      instance = TestFixtures.ordinary_component
      overlay = ctrl_drag_copy([instance], vector)

      overlay.onUserText('x3', @model.active_view)

      copies = @model.selection.to_a.reject { |e| e.equal?(instance) }
      assert_equal 3, copies.length, "for vector #{vector.to_a.inspect}"

      expected = [1, 2, 3].map { |i| ORIGIN.offset(vector, vector.length * i) }
      actual = copies.map { |c| c.transformation.origin }.sort_by { |p| p.distance(ORIGIN) }
      expected.sort_by! { |p| p.distance(ORIGIN) }

      actual.each_with_index do |point, i|
        assert_in_delta expected[i].x, point.x, 1e-6
        assert_in_delta expected[i].y, point.y, 1e-6
        assert_in_delta expected[i].z, point.z, 1e-6
      end
    end
  end

  # -- Rotation / scale / mirroring preservation ---------------------------

  def test_x3_preserves_rotation
    instance = TestFixtures.ordinary_component
    rotation = Geom::Transformation.rotation(ORIGIN, Z_AXIS, 30.degrees)
    instance.transformation = rotation
    overlay = ctrl_drag_copy([instance], Geom::Vector3d.new(600.mm, 0, 0))

    overlay.onUserText('x3', @model.active_view)

    assert_transformation_equal(rotation, instance.transformation, 'original rotation must be untouched')

    step = Geom::Transformation.translation(Geom::Vector3d.new(600.mm, 0, 0))
    copies = @model.selection.to_a.reject { |e| e.equal?(instance) }.sort_by { |c| c.transformation.origin.x }
    assert_equal 3, copies.length
    cumulative = IDENTITY
    copies.each do |copy|
      cumulative = step * cumulative
      assert_transformation_equal(cumulative * rotation, copy.transformation)
    end
  end

  def test_x2_preserves_non_uniform_scale
    instance = TestFixtures.ordinary_component
    scale = Geom::Transformation.scaling(ORIGIN, 2.0, 1.0, 0.5)
    instance.transformation = scale
    overlay = ctrl_drag_copy([instance], Geom::Vector3d.new(0, 300.mm, 0))

    overlay.onUserText('x2', @model.active_view)

    step = Geom::Transformation.translation(Geom::Vector3d.new(0, 300.mm, 0))
    copies = @model.selection.to_a.reject { |e| e.equal?(instance) }.sort_by { |c| c.transformation.origin.y }
    assert_equal 2, copies.length
    cumulative = IDENTITY
    copies.each do |copy|
      cumulative = step * cumulative
      assert_transformation_equal(cumulative * scale, copy.transformation)
    end
  end

  def test_x2_preserves_mirroring
    instance = TestFixtures.ordinary_component
    mirror = Geom::Transformation.scaling(ORIGIN, -1.0, 1.0, 1.0)
    instance.transformation = mirror
    overlay = ctrl_drag_copy([instance], Geom::Vector3d.new(100.mm, 0, 0))

    overlay.onUserText('x2', @model.active_view)

    step = Geom::Transformation.translation(Geom::Vector3d.new(100.mm, 0, 0))
    copies = @model.selection.to_a.reject { |e| e.equal?(instance) }.sort_by { |c| c.transformation.origin.x }
    assert_equal 2, copies.length
    cumulative = IDENTITY
    copies.each do |copy|
      cumulative = step * cumulative
      assert_transformation_equal(cumulative * mirror, copy.transformation)
    end
  end

  # -- Multi-selection: relative positions and mixed types -----------------

  def test_x4_preserves_relative_positions_for_multi_selection
    instance_a = TestFixtures.ordinary_component
    instance_b = TestFixtures.ordinary_component
    instance_b.transformation = Geom::Transformation.translation(Geom::Vector3d.new(80.mm, 0, 0))
    overlay = ctrl_drag_copy([instance_a, instance_b], Geom::Vector3d.new(400.mm, 0, 0))

    overlay.onUserText('x4', @model.active_view)

    a_copies = @model.selection.to_a.select { |e| e.definition.equal?(instance_a.definition) } - [instance_a]
    b_copies = @model.selection.to_a.select { |e| e.definition.equal?(instance_b.definition) } - [instance_b]
    assert_equal 4, a_copies.length
    assert_equal 4, b_copies.length

    ordered_a = a_copies.sort_by { |c| c.transformation.origin.x }
    ordered_b = b_copies.sort_by { |c| c.transformation.origin.x }
    ordered_a.zip(ordered_b).each do |a_copy, b_copy|
      assert_in_delta 80.mm, b_copy.transformation.origin.x - a_copy.transformation.origin.x, 1e-6
    end
  end

  def test_x2_mixed_group_and_component_instance_selection
    group = TestFixtures.group_box
    instance = TestFixtures.ordinary_component
    overlay = ctrl_drag_copy([group, instance], Geom::Vector3d.new(300.mm, 0, 0))

    overlay.onUserText('x2', @model.active_view)

    selected = @model.selection.to_a
    assert_equal 6, selected.length, '2 originals + (endpoint + 1 extra) each'
    group_copies = selected.select { |e| e.is_a?(Sketchup::Group) && !e.equal?(group) }
    instance_copies = selected.select do |e|
      e.is_a?(Sketchup::ComponentInstance) && !e.is_a?(Sketchup::Group) && !e.equal?(instance)
    end
    assert_equal 2, group_copies.length, 'the Group must be duplicated via Group#copy'
    assert_equal 2, instance_copies.length, 'the plain ComponentInstance must be duplicated via add_instance'
  end

  # -- Rejection: unsupported selection -------------------------------------

  def test_x3_rejects_unsupported_geometry_leaving_the_single_copy_intact
    group = TestFixtures.group_box
    edge = Sketchup::Edge.new(Geom::Point3d.new(0, 0, 0), Geom::Point3d.new(10, 0, 0))
    overlay = ctrl_drag_copy([group, edge], Geom::Vector3d.new(500.mm, 0, 0))
    before_count = @model.selection.to_a.length
    starts_before, commits_before, aborts_before = operation_counts

    overlay.onUserText('x3', @model.active_view)

    assert_equal before_count, @model.selection.to_a.length, 'the single Ctrl-drag copy must remain intact'
    starts_after, commits_after, aborts_after = operation_counts
    assert_equal starts_before, starts_after, 'a rejected array must never start an operation'
    assert_equal commits_before, commits_after
    assert_equal aborts_before, aborts_after
    assert_match(/groups and components/i, UI.last_messagebox_text.to_s)
  end

  def test_x3_rejects_glued_component
    instance = TestFixtures.ordinary_component
    instance.glued_to = Object.new
    overlay = ctrl_drag_copy([instance], Geom::Vector3d.new(500.mm, 0, 0))

    overlay.onUserText('x3', @model.active_view)

    assert_equal 1, @model.selection.to_a.length, 'only the single endpoint copy'
    assert_match(/glued/i, UI.last_messagebox_text.to_s)
  end

  # -- Repeated xN and switching between xN and /N --------------------------

  def test_x3_then_x4_replaces_the_array_and_never_moves_the_endpoint
    group = TestFixtures.group_box
    overlay = ctrl_drag_copy([group], Geom::Vector3d.new(1000.mm, 0, 0))
    endpoint = @model.selection.to_a.first

    overlay.onUserText('x3', @model.active_view)
    extras_after_x3 = session_of(overlay)[:intermediates].dup
    assert_equal 2, extras_after_x3.length

    overlay.onUserText('x4', @model.active_view)

    assert extras_after_x3.none?(&:valid?), 'the x3 extras must be replaced, not kept alongside x4'
    session = session_of(overlay)
    assert_equal 3, session[:intermediates].length, 'x4 means 4 total copies -- endpoint + 3 extras'
    assert_equal endpoint, session[:endpoint].first
    assert endpoint.valid?
    assert_in_delta 1000.mm, endpoint.transformation.origin.x, 1e-6, 'the endpoint must never move across re-edits'

    xs = session[:intermediates].map { |e| e.transformation.origin.x }.sort
    assert_in_delta 2000.mm, xs[0], 1e-6
    assert_in_delta 3000.mm, xs[1], 1e-6
    assert_in_delta 4000.mm, xs[2], 1e-6

    starts, commits, aborts = operation_counts
    assert_equal 3, starts, 'the initial drag-and-copy, then one start per "xN" edit'
    assert_equal 3, commits
    assert_equal 0, aborts

    transparent_starts = @model.operation_log.count { |e| e[0] == :start && e[4] == true }
    assert_equal 2, transparent_starts, 'each "xN" re-edit must request a transparent (undo-merging) operation'
  end

  def test_slash_3_then_x3_switches_to_the_external_array
    group = TestFixtures.group_box
    overlay = ctrl_drag_copy([group], Geom::Vector3d.new(3000.mm, 0, 0))
    endpoint = @model.selection.to_a.first

    overlay.onUserText('/3', @model.active_view)
    internal_extras = session_of(overlay)[:intermediates].dup
    assert_equal 2, internal_extras.length
    assert_equal :internal, session_of(overlay)[:mode]

    overlay.onUserText('x3', @model.active_view)

    assert internal_extras.none?(&:valid?), 'the /3 intermediates must be erased when switching to x3'
    session = session_of(overlay)
    assert_equal :external, session[:mode]
    assert_equal 3, session[:count]
    assert_equal 2, session[:intermediates].length
    assert endpoint.valid?
    assert_in_delta 3000.mm, endpoint.transformation.origin.x, 1e-6, 'the endpoint never moves, even across a mode switch'

    xs = session[:intermediates].map { |e| e.transformation.origin.x }.sort
    assert_in_delta 6000.mm, xs[0], 1e-6, 'external mode repeats the fixed drag vector, not the /3 subdivision'
    assert_in_delta 9000.mm, xs[1], 1e-6
  end

  def test_x3_then_slash_3_switches_back_to_the_internal_array
    group = TestFixtures.group_box
    overlay = ctrl_drag_copy([group], Geom::Vector3d.new(3000.mm, 0, 0))
    endpoint = @model.selection.to_a.first

    overlay.onUserText('x3', @model.active_view)
    external_extras = session_of(overlay)[:intermediates].dup
    assert_equal 2, external_extras.length
    assert_equal :external, session_of(overlay)[:mode]

    overlay.onUserText('/3', @model.active_view)

    assert external_extras.none?(&:valid?), 'the x3 extras must be erased when switching to /3'
    session = session_of(overlay)
    assert_equal :internal, session[:mode]
    assert_equal 3, session[:count]
    assert endpoint.valid?
    assert_in_delta 3000.mm, endpoint.transformation.origin.x, 1e-6

    xs = session[:intermediates].map { |e| e.transformation.origin.x }.sort
    assert_in_delta 1000.mm, xs[0], 1e-6, 'internal mode subdivides the fixed drag vector, not the x3 repetition'
    assert_in_delta 2000.mm, xs[1], 1e-6
  end

  # -- Mid-array failure: build-then-swap, never leaves a gap --------------

  def test_failure_during_copy_creation_leaves_the_existing_array_intact
    group = TestFixtures.group_box
    overlay = ctrl_drag_copy([group], Geom::Vector3d.new(1000.mm, 0, 0))
    overlay.onUserText('x3', @model.active_view)
    original_extras = session_of(overlay)[:intermediates].dup
    assert_equal 2, original_extras.length

    real_method = Zbellbound::SmartGizmoPro::GizmoOverlay.instance_method(:duplicate_supported_entity)
    call_count = 0
    overlay.define_singleton_method(:duplicate_supported_entity) do |entity|
      call_count += 1
      raise 'simulated duplication failure' if call_count == 2

      real_method.bind(overlay).call(entity)
    end

    original_stderr = $stderr
    $stderr = StringIO.new
    begin
      overlay.onUserText('x4', @model.active_view)
      captured = $stderr.string
    ensure
      $stderr = original_stderr
    end

    starts, commits, aborts = operation_counts
    assert_equal 1, aborts, 'a mid-array failure must abort, never commit a partial replacement'
    refute_nil UI.last_messagebox_text
    assert_empty captured, 'no Ruby Console output on a mid-array failure'

    session = session_of(overlay)
    assert_equal 3, session[:count], 'count must remain at the pre-failure value'
    assert_equal :external, session[:mode]
    assert_equal original_extras, session[:intermediates], 'the pre-failure extras must still be tracked'
    assert original_extras.all?(&:valid?), 'old extras must not be erased until the replacement set is fully built'
  end

  # -- Selection-observer timing protection ---------------------------------

  def test_a_delayed_notification_echoing_an_x_edits_own_selection_change_does_not_close_the_window
    group = TestFixtures.group_box
    overlay = ctrl_drag_copy([group], Geom::Vector3d.new(300.mm, 0, 0))
    overlay.onUserText('x3', @model.active_view)
    refute_nil session_of(overlay)

    # Simulates a SelectionObserver notification for the x3 edit's own
    # @selection.clear/@selection.add arriving only after the edit has
    # already returned (e.g. a later tick rather than synchronously).
    overlay.selection_changed(@model.selection)

    refute_nil session_of(overlay),
      'a notification that only echoes the edit\'s own selection change must not close the window'
  end

  def test_a_genuinely_different_selection_still_closes_the_window_after_an_x_edit
    group = TestFixtures.group_box
    overlay = ctrl_drag_copy([group], Geom::Vector3d.new(300.mm, 0, 0))
    overlay.onUserText('x3', @model.active_view)
    refute_nil session_of(overlay)

    other = Sketchup::Selection.new([TestFixtures.ordinary_component])
    overlay.selection_changed(other)

    assert_nil session_of(overlay), 'a real new selection must still close the window'
  end

  # -- Never reuses the temporary-grouping trick for array copies ----------

  def test_x_array_creation_never_uses_the_temporary_grouping_trick
    group = TestFixtures.group_box
    overlay = ctrl_drag_copy([group], Geom::Vector3d.new(300.mm, 0, 0))

    @model.active_entities.define_singleton_method(:add_group) do |*_args|
      raise 'add_group must never be called for Ctrl-drag array copies'
    end

    overlay.onUserText('x3', @model.active_view)

    assert_equal 4, @model.selection.to_a.length, 'x3 must still succeed without ever calling add_group'
  end

  # -- Metadata preservation --------------------------------------------------

  def test_x3_preserves_component_instance_metadata_and_attributes
    instance = TestFixtures.ordinary_component
    instance.name = 'Leg-01'
    instance.material = 'Oak'
    instance.layer = 'Furniture'
    instance.hidden = true
    instance.casts_shadows = false
    instance.receives_shadows = false
    instance.set_test_attribute_dictionary('zb_test', { 'sku' => 'ABC123' })
    overlay = ctrl_drag_copy([instance], Geom::Vector3d.new(300.mm, 0, 0))

    overlay.onUserText('x3', @model.active_view)

    copies = @model.selection.to_a.reject { |e| e.equal?(instance) }
    assert_equal 3, copies.length
    copies.each do |copy|
      assert_equal 'Leg-01', copy.name
      assert_equal 'Oak', copy.material
      assert_equal 'Furniture', copy.layer
      assert_equal true, copy.hidden?
      assert_equal false, copy.casts_shadows?
      assert_equal false, copy.receives_shadows?
      assert_equal 'ABC123', copy.attribute_dictionary('zb_test')['sku']
    end
  end

  def test_x3_preserves_group_metadata_and_attributes
    group = TestFixtures.group_box
    group.name = 'Frame-Left'
    group.material = 'Steel'
    group.layer = 'Structure'
    group.hidden = true
    group.casts_shadows = false
    group.receives_shadows = false
    group.set_test_attribute_dictionary('zb_test', { 'part' => 'frame' })
    overlay = ctrl_drag_copy([group], Geom::Vector3d.new(0, 300.mm, 0))

    overlay.onUserText('x3', @model.active_view)

    copies = @model.selection.to_a.reject { |e| e.equal?(group) }
    assert_equal 3, copies.length
    copies.each do |copy|
      assert_equal 'Frame-Left', copy.name
      assert_equal 'Steel', copy.material
      assert_equal 'Structure', copy.layer
      assert_equal true, copy.hidden?
      assert_equal false, copy.casts_shadows?
      assert_equal false, copy.receives_shadows?
      assert_equal 'frame', copy.attribute_dictionary('zb_test')['part']
    end
  end

  # -- Ordinary Ctrl-drag copy unaffected ------------------------------------

  def test_ordinary_ctrl_drag_single_copy_unchanged_without_array_input
    group = TestFixtures.group_box
    overlay = ctrl_drag_copy([group], Geom::Vector3d.new(1000.mm, 0, 0))

    selected = @model.selection.to_a
    assert_equal 1, selected.length, 'only the endpoint copy, matching ordinary Ctrl-drag copy'
    assert group.valid?
    assert_in_delta 0, group.transformation.origin.x, 1e-6
    refute_nil session_of(overlay), 'the array-input window must be open, ready for either syntax'
  end

  # -- No Ruby Console output on success ------------------------------------

  def test_successful_x_array_produces_no_console_output
    group = TestFixtures.group_box
    overlay = ctrl_drag_copy([group], Geom::Vector3d.new(300.mm, 0, 0))

    original_stderr = $stderr
    $stderr = StringIO.new
    begin
      overlay.onUserText('x3', @model.active_view)
      captured = $stderr.string
    ensure
      $stderr = original_stderr
    end

    assert_empty captured
  end
end
